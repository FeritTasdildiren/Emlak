"""
Ankara Konut Piyasasi - LightGBM Egitim Verisi Hazirlama Scripti

ankara_districts.json ilce verileri kullanilarak sentetik egitim verisi uretir.
Istanbul prepare_training_data.py ile ayni format ve mantik.

Cikti: ankara_training_data.csv
"""
...
# ── Sabitler ──────────────────────────────────────────────────────────
SEED = 43  # Ankara icin deterministik seed
MIN_RECORDS_PER_DISTRICT = 80
MAX_RECORDS_PER_DISTRICT = 120

SCRIPT_DIR = Path(__file__).resolve().parent
INPUT_JSON = SCRIPT_DIR / "ankara_districts.json"
OUTPUT_CSV = SCRIPT_DIR / "ankara_training_data.csv"
...
# Isitma tipleri — Ankara daha soguk, dogalgaz orani yuksek
HEATING_TYPES = ["Dogalgaz_Kombi", "Merkezi", "Dogalgaz_Kat", "Soba", "Klima", "Yerden_Isitma"]
HEATING_WEIGHTS = [0.45, 0.22, 0.15, 0.08, 0.03, 0.07]
...
def main() -> None:
    ...
    print(f"[INFO] {len(districts)} ilce yuklendi (Ankara).")
