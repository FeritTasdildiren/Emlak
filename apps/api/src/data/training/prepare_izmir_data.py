"""
Izmir Konut Piyasasi - LightGBM Egitim Verisi Hazirlama Scripti

izmir_districts.json ilce verileri kullanilarak sentetik egitim verisi uretir.
Istanbul prepare_training_data.py ile ayni format ve mantik.

Cikti: izmir_training_data.csv
"""
...
# ── Sabitler ──────────────────────────────────────────────────────────
SEED = 44  # Izmir icin deterministik seed
MIN_RECORDS_PER_DISTRICT = 80
MAX_RECORDS_PER_DISTRICT = 120

SCRIPT_DIR = Path(__file__).resolve().parent
INPUT_JSON = SCRIPT_DIR / "izmir_districts.json"
OUTPUT_CSV = SCRIPT_DIR / "izmir_training_data.csv"
...
# Isitma tipleri — Izmir Ege iklimi, klima orani biraz daha yuksek
HEATING_TYPES = ["Dogalgaz_Kombi", "Merkezi", "Dogalgaz_Kat", "Soba", "Klima", "Yerden_Isitma"]
HEATING_WEIGHTS = [0.38, 0.18, 0.14, 0.08, 0.12, 0.10]
...
# CSV sutun siralamasi (Istanbul ile ayni)
CSV_COLUMNS = [
    "district",
    "neighborhood",
    "property_type",
...
def _calculate_price(
    ...
    """
    Fiyat hesaplama formulu (Istanbul ile ayni):
...
def main() -> None:
    ...
    print(f"[INFO] {len(districts)} ilce yuklendi (Izmir).")
